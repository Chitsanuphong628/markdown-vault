...
import { getNativeOAuthMetadata } from "./oauth";
...
export function oauthMetadata(requestUrl?: string): OAuthMetadata | null {
  ...
  // Fallback to Native Built-in OAuth 2.0 Server
  if (requestUrl) {
    return getNativeOAuthMetadata(requestUrl);
  }
  const appUrl = process.env.NEXT_PUBLIC_APP_URL;
  if (appUrl) {
    return getNativeOAuthMetadata(appUrl.startsWith("http") ? appUrl : `https://${appUrl}`);
  }
  return null;
}
...
async function authInfoForToken(token: string, resource: URL): Promise<AuthInfo> {
  ...
  const metadata = oauthMetadata(resource.origin);
...
export async function serveMcpHttp(request: Request): Promise<Response> {
  ...
  const metadata = oauthMetadata(expected.origin);
...
export function serveMcpOAuthMetadata(request: Request): Response {
  const effectiveUrl = getEffectiveRequestUrl(request);
  if (process.env.ENABLE_MCP !== "true" || !isTrustedAppUrl(effectiveUrl)) return new Response(null, { status: 404 });
  const expected = resourceUrl(effectiveUrl);
  const metadata = oauthMetadata(expected.origin);
  if (!metadata) return new Response(null, { status: 404 });
  return oauthMetadataResponse(request, {
    oauthMetadata: metadata, resourceServerUrl: expected, scopesSupported: ["mcp"], resourceName: "Nota Vault",
